def control_positions(plate_dims, control_count):
    ...
    cntrl_pos = np.zeros(plate_dims, dtype=bool)

    assert control_count >= 12, "For placing controls on the edge, atleast 12 controls are required"

    row_pos1 = [round(i-1) for i in np.linspace(1, plate_dims[0], 4)]
    row_pos2 = [round(i-1) for i in np.linspace(1, plate_dims[0], 3)]
    col_pos = [round(i-1) for i in np.linspace(1, plate_dims[1], 4)]

    if control_count<14:
        print 'Only 6 controls on th edge, would be better with atleast 14 in controls in total'
